...
/*
	Note:
	    ...
	    Using (dynamic, num_threads) scheduling in OpenMP.

	Compile via:
		g++ -g -fopenmp -o sequences-dynamic sequences-dynamic.cpp etime.c -std=c++0x
		-std+c++0x for stoi()

	Execute via:
		. /sequences-dynamic numOfThreads(Int) motifsFile.txt sequencesFile.txt outputFile.txt
...
//	} cout << endl << endl;
	...
	*/
	...
	//OpenMP
	# pragma omp parallel for num_threads(thread_count) schedule(dynamic, thread_count)
